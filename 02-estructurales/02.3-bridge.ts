...
class AllertNotificacion extends Notification {
  override notify(message: string): void {
    console.log(`%cEnviando alerta: ${message}`, COLORS.red);
    this.channels.forEach((channel) => channel.send(message));
  }
  override addChannel(channel: NotificationChannel): void {
    this.channels.push(channel);
    console.log(`%cCanal agregado: ${channel.constructor.name}`, COLORS.green);
  }
}
...
function main() {
  ...
  const alertNotification = new AllertNotificacion(channels);

  alertNotification.notify("¡Alerta de seguridad activada!");
